...
public class FollowService {
  ...
  public boolean followingUnfollowing(Long memberId, Long atelierId) {
    ...
    if (followRepository
        .existsByMemberIdAndAtelierIdAndUseFlagIsTrue(member.getId(), atelier.getId())) {
  ...
  public CustomPageResponse<ReadFollowsResponse> getAllFollowings(Long memberId,
      Pageable pageRequest) {
    Member member = memberRepository.findById(memberId)
        .orElseThrow(() -> new BusinessException(ErrorInfo.MEMBER_NOT_FOUND));

    Page<ReadFollowsResponse> readAllFollowingResponses =
        followRepository.findAllByMemberIdAndUseFlagIsTrue(member.getId(), pageRequest)
            .map(followConverter::convertToReadFollowsResponse);

    return CustomPageResponse.<ReadFollowsResponse>of(readAllFollowingResponses);
  }
